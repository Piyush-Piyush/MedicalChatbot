...
# Reduced data by prognosis group
...
# Mapping symptoms to index
for index, symptom in enumerate(x):
    symptoms_dict[symptom] = index

# Function to calculate the condition severity
...
def getprecautionDict():
    ...
    with open('./symptom_precaution.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
            precautionDictionary.update({row[0]: [row[1], row[2], row[3], row[4]]})
...
# Secondary prediction for symptoms
...
def tree_to_code(tree, feature_names):
    ...
    while True:
        ...
        if conf == 1:
            ...
            for num, it in enumerate(cnf_dis):
                print(num, ")", it)
            if num != 0:
                readn("select the one you meant based on number 0 to {}".format(num))
    ...
    while True:
        try:
            ...
            num_days = int(input("Okay. From how many days are you experiencing  ? : "))
            break
        except:
            readn("Enter valid symptom.")
    ...
    def recurse(node, depth):
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            readn("are you experiencing any of the following")
            symptoms_exp = []
            ...
            if present_disease[0] == second_prediction[0]:
                ...
                readn("You may have, {}".format(present_disease[0]))
